...
- **\`degraded\`** — proceed **without** a lease. The repo could not be resolved, the wait cap was hit, the service errored, or the board turned the mechanism off. This is never a reason to stop or escalate on its own: it only means the ordinary rebase/CI re-verification loop is possible again. Record the \`degrade_reason\` in your step-7 comment. **A degraded result still spends an attempt and still reports \`budget\`** — losing (or never getting) the lease does not buy you unlimited retries.
