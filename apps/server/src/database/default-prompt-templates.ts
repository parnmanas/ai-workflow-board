...
4. **Decision**:
   ...
   - **Wait** → \`add_comment\` with the waiting reason and which ticket you are waiting on. Then register the block so it's visible to the system, not just prose: \`mcp__awb__add_ticket_prerequisites(ticket_id="<this ticket>", prerequisite_ticket_ids=["<full prerequisite ticket UUID>"], reason="apps/client/src/components/chat/* overlap — waiting for <full prerequisite ticket UUID> to land")\` — \`prerequisite_ticket_ids\` takes the ticket's real full UUID, never a shortened id. Do **not** \`move_ticket\`. Do **not** \`pend_ticket\` — \`pend_ticket\` is for a blocker only a human can resolve; waiting on your own concurrent work is another *ticket* finishing, which is exactly what \`add_ticket_prerequisites\` is for (rule of thumb: human answer → \`pend_ticket\`; another ticket finishing → \`add_ticket_prerequisites\`). It's still self-resolving — nobody needs to unblock you by hand — but it must be registered, not left as a bare comment: registering sets \`pending_on_tickets=true\`, which the stuck/idle detectors read as an intentional wait, so the ticket isn't misjudged as idle and redispatched while you sit on it. It also **auto-resumes**: the moment the named ticket lands on a terminal column, the block clears and this ticket's current-column role holders are re-triggered automatically — no \`unpend_ticket\` or manual re-check needed. Don't repeat the identical "still waiting" note on every re-trigger; the prerequisite link now carries that state, and its resolution is what brings you back.
...
`,
  },
  {
    ...
    description: 'Merging column default workflow — assignee rebases the feature branch onto the latest default and actively integrates same-meaning conflicts (escalating only on a genuinely big problem), dispatches a pre-landing CI run on the rebased branch (or records a narrow exemption), lands it, deletes the branch (local + remote), bumps parent submodule ref if needed, and moves the ticket to Done.',
    category: 'default_workflow',
...
2. **Rebase onto the latest default and integrate**
   ...
   - **Dispatch CI before landing** — after that push, run the pre-landing CI check in "CI dispatch before landing" below and wait for its result before moving to step 3. A clean fast-forward is not itself evidence the branch is safe to land — only a completed green run (or a recorded exemption) is.
...
7. **Ticket comment** — \`add_comment\` with all of, **pasting the actual command output** (not a bare "OK" — Done independently re-checks these, and a no-repo reporter can only audit what you paste):
   ...
   - **CI dispatch result** — the run URL + conclusion from "CI dispatch before landing" below, or the exemption reason you recorded. Never omit both.
...
- **Dispatch** — target the rebased feature branch (the ref, not the default):
  ...
  - Otherwise, use the REST API directly with a resolved token (never echo the token itself):
    \`\`\`
    TOK=$(printf 'protocol=https\\nhost=github.com\\n\\n' | git credential fill | grep -oP '^password=\\K.*')
...
- **Find *your* run — match by SHA, never by recency.** A dispatch call does not return a run id, and run creation is asynchronous: immediately after dispatch, the newest run on that branch/event can still be the *previous* dispatch's run, not yours. Under the rebase-and-retry loop in step 3 ("this loop is normal under concurrent merges"), that stale run is very often \`success\` — polling by recency alone would report green without your rebased commit ever having been tested, landing un-CI'd code. Match on the SHA you captured instead:
  - \`gh\`: \`gh run list --workflow=ci.yml --branch=<feature-branch> --event=workflow_dispatch --limit=10 --json databaseId,headSha,status,conclusion\`, then select the entry whose \`headSha == $SHA\`. If none exists yet, the run hasn't been created — briefly re-list (run *creation* is seconds, not minutes); do not fall back to the newest entry.
  - REST fallback: \`GET /repos/<owner>/<repo>/actions/runs?branch=<feature-branch>&event=workflow_dispatch&per_page=10\`, filter \`workflow_runs[]\` for \`head_sha == $SHA\`.
- **Register a durable wait — never poll, sleep, or call ScheduleWakeup in this session.** The full matrix (6 jobs incl. \`agent-manager tests (windows-latest)\`) can take several minutes; a live session blocked that long on \`sleep\`/\`gh run watch\`/\`ScheduleWakeup\` has repeatedly died mid-wait (clean exit or crash) and left the ticket needing manual recovery (ticket 778b6dc7 — \`ScheduleWakeup\` in particular is a CLI-harness \`/loop\`-only tool with no AWB concept, and does not survive session exit anyway). Once you have the run id from the step above, call \`mcp__awb__await_ci_run(ticket_id, owner, repo, run_id, head_sha)\` — it records the wait durably (audit trail + run/SHA context) and returns immediately. Then \`add_comment\` noting the run URL and that you're awaiting its result, and **end this turn**. \`CiWaitResumeService\` polls the run server-side and automatically re-dispatches this ticket's assignee — with the result already posted as a comment — the moment the run resolves (or after a bounded timeout if it never does); continue from "Result" below in that resumed turn. If the run has already completed by the time you call this (rare, but possible for a fast run), the resume still happens on the next sweep, typically within a couple of minutes.
- **Result** (this turn, or the turn \`CiWaitResumeService\` wakes you for):
  - **\`success\`** → proceed to step 3; note the run URL + conclusion in the step-7 comment.
  - **Anything else** (\`failure\`, \`cancelled\`, \`timed_out\`) → do **not** proceed to step 3. Treat it exactly like a post-integration test failure under "When to integrate vs. escalate" above: an obvious, mechanical fix → fix it, re-push, re-dispatch (capturing a fresh SHA and calling \`mcp__awb__cancel_ci_wait\` first if the prior wait is still registered); otherwise \`move_ticket\` back to In Progress (or \`pend_ticket\`) with the run URL and failing job name(s) in the comment.
...
- **\`queued\`** — another ticket is landing right now. You are parked (\`pending_merge_lease\`, the same durable-wait shape as \`await_ci_run\`). **Leave a one-line comment saying you are queued, then end your turn.** Do **not** poll, sleep, or call ScheduleWakeup — the server re-dispatches this ticket's current-column role holders the moment the lease becomes yours. \`position\` and \`ahead_ticket_id\` in the response say where you are in line. Queueing costs no attempt (you are not running CI), and the queue is strict FIFO by arrival — re-calling early cannot move you up the line, and a ticket arriving after you cannot overtake you.
- **\`degraded\`** — proceed **without** a lease. The repo could not be resolved, the wait cap was hit, the service errored, or the board turned the mechanism off. This is never a reason to stop or escalate on its own: it only means the ordinary rebase/CI re-verification loop is possible again. Record the \`degrade_reason\` in your step-7 comment. **A degraded result still spends an attempt and still reports \`budget\`** — losing (or never getting) the lease does not buy you unlimited retries. The one exception is \`degrade_reason: "budget_unavailable"\`, which means the server could not even record the attempt: with no counter there is no way to promise the loop ends, so it is reported as \`budget: "exhausted"\` — treat it exactly like an exhausted budget and stop rather than retry.

**The retry budget is bounded, and exhausting it is an explicit failure.** A lease serializes AWB's own tickets; it cannot stop a human pushing straight to the base, or a second AWB instance watching the same repo. So **every** call that clears you to proceed — \`granted\` *or* \`degraded\` — spends one attempt, and the response reports \`attempt\` / \`max_attempts\` / \`budget\`.

The budget belongs to this ticket's **landing episode**, not to any one lease: it survives losing the lease, waiting the cap out, and every degraded result, and it resets only when the ticket actually leaves Merging (landed, bounced, or moved elsewhere). That is deliberate — a budget that reset each time you re-queued would let a busy repo loop forever.

When \`budget\` comes back \`"exhausted"\`, **stop — do not rebase again.** Comment with how many attempts you made and what kept moving the base, then bounce to **In Progress** or \`pend_ticket\`. Ending explicitly is the intended outcome; looping is not.

**You almost never release by hand.** *Moving* out of Merging — to Done after landing, or bounced back to In Progress — releases the lease inside the *same* column-move transaction, server-side. \`pend_ticket\` does not move the ticket, so it releases a beat later instead: the server sees a ticket blocked on a human and reclaims the lease on its next sweep (well under a minute), because nothing waiting on a person should hold the whole repo's landing window. Call \`mcp__awb__release_merge_lease(ticket_id)\` only when you are abandoning the landing attempt but staying in Merging under none of those conditions. And if your session simply dies holding the lease, the server reclaims it once there is no sign of progress — an unresolved \`await_ci_run\` **counts** as progress, so a slow CI matrix never costs you the lease mid-run.
...
- **CI dispatch is mandatory unless explicitly exempted.** See "CI dispatch before landing" above — landing without a green run or a recorded exemption reopens the gap ticket 34a6281a closed.
