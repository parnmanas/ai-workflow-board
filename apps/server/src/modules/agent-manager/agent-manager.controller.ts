...
export class AgentManagerController {
  ...
  async heartbeat(@Body() body: any, @Req() req: Request, @Res() res: Response) {
    ...
    // Self-update metadata. `latest_version` is whatever the manager's
    // upstream-version probe found on the tracked branch; can lag the
    // current heartbeat by up to one probe interval. `update_available` is
    // computed by the manager (it has the comparison context) so the server
    // doesn't try to second-guess version-string semantics. `repo_root` is
    // surfaced so the operator can see where an Update would run.
    const latest_version = typeof body?.latest_version === 'string' && body.latest_version
      ? body.latest_version
      : undefined;
    const update_available = typeof body?.update_available === 'boolean'
      ? body.update_available
      : undefined;
    const repo_root = typeof body?.repo_root === 'string' && body.repo_root
      ? body.repo_root
      : undefined;
