  ...
  server.tool(
    ...
    'Update a board name, description, routing_config, column→prompt-template mapping, auto-archive policy, agent harness override, or output language',
    {
      ...
      effort_presets: EffortPresetsConfigSchema.nullable().optional()
        .describe('Per-board abstract effort preset catalog: { default: <preset id>, presets: [{ id, label, claude?: { effort?, ultracode?, model? }, codex?: { model? }, antigravity?: { model? }, pi?: { model? } }] }. A ticket carries an abstract preset id (effort_preset); dispatch resolves it against this catalog and agent-manager maps the matched preset onto per-CLI options. Pass null to clear (board falls back to the built-in catalog).'),
