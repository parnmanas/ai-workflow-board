// Behavioral test for SecurityScheduleService.runOnce() — drives the scheduler
// sweep against in-memory fake repos with a fixed `now` (ticket 7c07c19d). Direct
// sibling of qa-schedule-behavior.test.mjs. Covers the acceptance points from the
// ticket's 검증 section plus the overlap policy:
//
...
//   • scope='selected' → startBatch({profileIds}) with exactly the listed ids.
...
function svcWith(rows, batches = []) {
  ...
  const svc = new SecurityScheduleService(scheduleRepo, batchRepo, runService, noopLog);
...
test('idempotency: a second sweep at the same `now` re-dispatches nothing (cursor advanced)', async () => {
  ...
  assert.equal(runService.calls.length, 1, 'startBatch still called exactly once total');
});

test("scope='selected' dispatches startBatch with exactly the listed profile ids", async () => {
  const sch = makeSchedule({ scope: 'selected', profile_ids: ['p-a', 'p-b'], interval_ms: 10 * MIN });
  const { svc, runService } = svcWith([sch]);

  await svc.runOnce(NOW);
  assert.equal(runService.calls.length, 1);
  const { args } = runService.calls[0];
  assert.deepEqual(args.profileIds, ['p-a', 'p-b'], 'only the selected ids run');
...
test('disabled schedule is never swept even when overdue', async () => {
  ...
  assert.equal(runService.calls.length, 0, 'startBatch never called');
});
...
test('a finished previous batch does NOT block the next dispatch', async () => {
  const sch = makeSchedule({ last_batch_id: 'prev-batch' });
  const { svc, runService } = svcWith([sch], [{ id: 'prev-batch', status: 'done' }]);
...
test('runNow fires regardless of enabled and does NOT disturb next_run_at', async () => {
  ...
  assert.equal(runService.calls.length, 1, 'startBatch called by run-now even though disabled');
