 ...
 *     에이전트 자신의 진행 보고다. 활동이 방금 찍혔는데도 이 시계는 허용 시간을 향해
 *     계속 흐를 수 있고, 그 어긋남이 2026-09-25 EmberDelve 에서 열심히 일한 step 이
 *     100분 뒤 lease 만료로 실패한 이유였다.
 *
 * 판정은 `step-activity.ts` 의 순수 함수에 있다 — 좌측 레일이 같은 판정을 쓰기 때문에
 * (같은 step 이 두 곳에서 다른 상태로 읽히면 안 된다) 여기서는 그리기만 한다.
 */
function LiveActivity({
  ...
  const now = Date.now();
  const view = describeStepActivity(step, now);
  const quiet = describeStepQuiet(step, stepTimeoutMinutes, now);
  const running = runningLabel(view);
  ...
  return (
    <div
      ...
      style={{
        ...
        padding: '5px 6px',
        borderRadius: 6,
        background: view.stalled ? `${tokens.colors.warningBg}40` : `${tokens.colors.border}45`,
        borderLeft: `2px solid ${view.stalled ? tokens.colors.warningLight : tokens.colors.infoLight}`,
      }}
    >
      {view.text ? (
        <>
        ...
        <div
          style={{
            fontSize: 10,
            color: view.stalled ? tokens.colors.warningLight : tokens.colors.textMuted,
            lineHeight: 1.35,
          }}
          title={
            view.stalled
        ...
        >
          {compactActivityLabel(view)}
          {view.runningFor !== null && ` · ${shortDuration(view.runningFor)}`}
        </div>
      )}
      {quiet && (
        <div
