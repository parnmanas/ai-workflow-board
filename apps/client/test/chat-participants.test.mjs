...
// import 하는 participantFlow.ts 를 그대로 구동한다. 순수 헬퍼(가드/후보 제외)뿐 아니라
// 컴포넌트의 "연결부"까지 이 모듈로 추출돼 있어(dispatchChatRoomUpdate = ChatPage 의
// chat_room_update 핸들러 본문, loadAddPeopleCandidates = ParticipantPicker 의 open
// effect 로드 본문), 컴포넌트는 ref/세터만 주입한다. 따라서 참여자 SSE 분기를 지우거나
// (participant_left 만 누락, 다른 분기로 오배선) 후보 목록을 잘못 set 하면 이 테스트가
// 실패한다 — 컴포넌트에 남는 미검증 잔여물은 useBoardStreamEvent 등록/DI 리터럴 뿐이다
// (그 React glue 는 jsdom 풀마운트가 필요해 이 레포 범위 밖 — README 참조).
//
...
// (getChatRoom / listChatRooms / getUsers / getAgents / setter)을 받으므로, 아래
// deferred() 로 응답을 "임의 순서로" 완료시켜 방 전환 경합을 결정적으로 재현한다.
// 최소 시드 = 아래 user()/agent()/roomDetail()/roomListItem() 팩토리가 만드는 인메모리 픽스처.
...
const roomListItem = (id, extra = {}) => ({ id, type: 'group', name: id, unread_count: 0, ...extra });

/**
 * React useState 세터를 흉내낸 rooms 상태 홀더. dispatchChatRoomUpdate 의 setRooms 는
 * 값(reflectParticipantChange 경로) 또는 updater 함수(renamed/read 경로) 둘 다 받는다.
 */
function makeRoomsState(initial = []) {
  let rooms = initial;
  return {
    setRooms: (update) => {
      rooms = typeof update === 'function' ? update(rooms) : update;
    },
    get: () => rooms,
  };
}
...
// ─── 시나리오 2b: dispatchChatRoomUpdate — ChatPage 의 SSE 연결부 ───────────────
// ChatPage 의 chat_room_update 핸들러 본문이 그대로 이 함수로 추출돼 있어, 실제
// 이벤트 페이로드(봉투/flat)로 "봉투 unwrap + update_type 분기" 배선을 직접 구동한다.
// (컴포넌트에서 participant 분기를 지우거나 오배선하면 아래 단언이 깨진다.)

test('dispatchChatRoomUpdate: participant_added(활성 방, 서버 봉투 shape) → 방 목록 + 활성 로스터 재조회', async () => {
  let activeRoomId = 'A';
  const roomsState = makeRoomsState([roomListItem('A')]);
  const listResult = [roomListItem('A'), roomListItem('B')];
  let listCalls = 0;
  let roster = null;
  ...
  dispatchChatRoomUpdate(
    ...
    { payload: { update_type: 'participant_added', room_id: 'A' } }, // { event_type, payload, ... } 봉투
  );

  await flush();
  assert.equal(listCalls, 1, '봉투 payload 를 unwrap 해 participant 분기로 방 목록 재조회');
  assert.deepEqual(roomsState.get().map((r) => r.id), ['A', 'B'], '방 목록 갱신 반영');
  assert.deepEqual(roster.map((p) => p.id), ['a1', 'a2'], '활성 방이므로 상단 로스터도 재조회');
});

test('dispatchChatRoomUpdate: participant_left(활성 방, flat shape) → 로스터도 재조회 (리뷰 지적 갭)', async () => {
  // 완료 조건의 "participant_left 이벤트 후 활성 방 로스터 갱신"을 직접 단언한다
  // (기존 테스트는 participant_left 를 비활성 방으로만 검증했다).
  let activeRoomId = 'A';
  const roomsState = makeRoomsState([]);
  let rosterRefreshedFor = null;

  dispatchChatRoomUpdate(
    {
      currentUserId: 'u-self',
      getActiveRoomId: () => activeRoomId,
      listChatRooms: () => Promise.resolve([roomListItem('A')]),
      setRooms: roomsState.setRooms,
      refreshActiveRoomParticipants: (roomId) => {
        rosterRefreshedFor = roomId;
      },
    },
    { update_type: 'participant_left', room_id: 'A' }, // 봉투 없는 flat payload
  );

  await flush();
  assert.equal(rosterRefreshedFor, 'A', 'participant_left 도 활성 방이면 상단 로스터 재조회');
  assert.deepEqual(roomsState.get().map((r) => r.id), ['A'], '방 목록도 갱신');
});

test('dispatchChatRoomUpdate: participant_left(비활성 방) → 방 목록만 갱신, 로스터 미변경', async () => {
  let activeRoomId = 'A';
  let listCalls = 0;
  let rosterRefreshed = false;
  ...
  dispatchChatRoomUpdate(
    ...
    { payload: { update_type: 'participant_left', room_id: 'B' } }, // 이벤트는 B, 활성은 A
  );

  await flush();
  assert.equal(listCalls, 1, '어느 방 이벤트든 방 목록은 재조회');
  assert.equal(rosterRefreshed, false, '이벤트 방(B)이 비활성이면 로스터는 건드리지 않음');
});

test('dispatchChatRoomUpdate: renamed 는 방 이름만 갱신하고 참여자 재조회를 트리거하지 않는다 (분기 게이팅)', async () => {
  const roomsState = makeRoomsState([roomListItem('A', { name: 'old' })]);
  let listCalls = 0;
  let rosterRefreshed = false;
  ...
  dispatchChatRoomUpdate(
    ...
    { payload: { update_type: 'renamed', room_id: 'A', new_name: 'new' } },
  );

  await flush();
  assert.equal(roomsState.get()[0].name, 'new', 'renamed → 해당 방 이름 갱신');
  // participant 분기로 새면 listChatRooms 가 호출된다 — 0 이어야 게이팅이 산다.
  assert.equal(listCalls, 0, 'renamed 는 participant 재조회 경로를 타지 않음');
  assert.equal(rosterRefreshed, false, 'renamed 는 로스터 재조회 안 함');
});

test('dispatchChatRoomUpdate: read(본인) 는 해당 방 unread 를 0 으로, 참여자 재조회는 안 함', async () => {
  const roomsState = makeRoomsState([roomListItem('A', { unread_count: 5 })]);
  let listCalls = 0;
  ...
  dispatchChatRoomUpdate(
    ...
    { payload: { update_type: 'read', room_id: 'A', participant_type: 'user', participant_id: 'u-self' } },
  );

  await flush();
  assert.equal(roomsState.get()[0].unread_count, 0, 'read(본인) → unread 0');
  assert.equal(listCalls, 0, 'read 는 participant 재조회 경로가 아님');
});

test('dispatchChatRoomUpdate: read(다른 사용자) 와 미지의 update_type 은 무시된다', async () => {
  const roomsState = makeRoomsState([roomListItem('A', { unread_count: 5 })]);
  let listCalls = 0;
  let rosterRefreshed = false;
  const deps = {
  ...
  // 다른 사용자 read → 내 unread 를 건드리지 않음 (participant_id !== currentUserId).
  dispatchChatRoomUpdate(deps, {
    payload: { update_type: 'read', room_id: 'A', participant_type: 'user', participant_id: 'someone-else' },
  });
  // 미지의 타입 + null payload → no-op, throw 없음.
  dispatchChatRoomUpdate(deps, { payload: { update_type: 'poked', room_id: 'A' } });
  dispatchChatRoomUpdate(deps, null);

  await flush();
  assert.equal(roomsState.get()[0].unread_count, 5, '다른 사용자 read 는 내 unread 를 바꾸지 않음');
  assert.equal(listCalls, 0, '무관한 이벤트는 방 목록 재조회를 트리거하지 않음');
  assert.equal(rosterRefreshed, false, '무관한 이벤트는 로스터 재조회를 트리거하지 않음');
});
...
// loadAddPeopleCandidates = ParticipantPicker 의 open effect 로드 본문(추출). 실제
// getUsers/getAgents fetch → 후보 빌드 → setParticipants 배선을 구동한다. 컴포넌트가
// 후보 대신 잘못된 목록을 set 하는 오배선(리뷰 지적)을 이 테스트가 잡는다.

test('loadAddPeopleCandidates: users+agents 로드 후 기존·본인·매니저 제외한 후보를 set', async () => {
  let captured = null;
  await loadAddPeopleCandidates({
    getUsers: () =>
      Promise.resolve([
        { id: 'u-self', name: 'Me' },
        { id: 'u1', name: 'Alice' },
  ...
  assert.deepEqual(captured.map((p) => p.id), ['u2', 'a1'], '기존(u1)·본인(u-self)·매니저(a2) 제외');
  assert.deepEqual(
    captured.find((p) => p.id === 'a1'),
    { id: 'a1', name: 'Bot1', type: 'agent' },
    'PickerParticipant shape 로 매핑',
  );
});

test('loadAddPeopleCandidates: getAgents 실패해도 users 로 폴백해 후보를 set (네트워크 방어)', async () => {
  let captured = null;
  await loadAddPeopleCandidates({
    getUsers: () => Promise.resolve([{ id: 'u1', name: 'Alice' }]),
    getAgents: () => Promise.reject(new Error('boom')),
    currentUserId: null,
    formatAgentName: (a) => a.name,
    setParticipants: (list) => {
      captured = list;
  ...
  assert.deepEqual(captured.map((p) => p.id), ['u1'], 'agents 조회 실패 → [] 폴백, users 만 후보');
});
