...
export interface SelfUpdateOpts {
   ...
   * 게이트 입력). 창 자체를 정의하는 `AWB_AGENT_MANAGER_UPDATE_WINDOW` 는 형제
   * 티켓 T1 의 범위라 여기서 도입하지 않는다 — 생략하면 `true`(항상 창 안)로
   * 다뤄져 현행 동작이 그대로 유지된다.
   */
...
async function runNpmGlobalSelfUpdate(
  ...
  if (trackable) {
    const retry = evaluateInstallRetryGate({
      ...
      nowMs,
      // 유지보수 창 판정은 형제 티켓 T1 의 범위다. 미지정이면 항상 창 안 —
      // 창이 없는 현행 동작과 동등하다.
      withinWindow: opts.withinWindow ?? true,
    });
  ...
  if (process.platform !== 'win32') {
    const installed = await npmGlobalInstall(out, installSpec);
    if (!installed.ok) {
    ...
    // 설치 성공 — 이제부터는 "새 빌드가 하트비트 1회를 성공시키는가"가 남은
    // 판정이다. 이 표시가 있어야 다음 부팅이 자신을 검증 대상으로 인식한다.
    if (installRecord) {
...
async function runRollbackInstall(
  ...
  const verdict = await verifyNpmGlobalProvenance(out, target);
  ...
  if (process.platform !== 'win32') {
    const installed = await npmGlobalInstall(out, installSpec);
    if (!installed.ok) {
      // 매니저는 계속 돌고 있다(완료 기준 7). 핀도 그대로라 불량 버전으로
      // 되돌아가지 않는다. 다음 부팅이 rollbackAttempts 를 보고 재시도하거나
      // 상한에서 멈춘다.
    ...
    scheduleRollbackRestart(out, () => reExecManager(out));
    return {
      kind: 'rollback',
      armed: false,
      willReExec: true,
  ...
  try {
    const child = spawn(
      process.execPath,
      [helperPath, String(process.pid), installSpec, process.execPath, process.argv[1] || '', ...baseArgs],
 ...
 */
function scheduleRollbackRestart(out: (msg: string) => void, restart: () => void): void {
  _lastReExecScheduled = true;
  _pendingRestartReason = 'self_update_restart';
