...
export function createBuiltinRuntimeRegistry(extensions: readonly RuntimePluginManifest[] = []): RuntimePluginRegistry {
  const registry = new RuntimePluginRegistry()
    ...
    .register(defineRuntimePlugin({ id: 'codex', transport: 'cli', capabilities: requestCapabilities(oneshot(true, 'tokens'), { streaming: true }), createCliAdapter: () => new CodexCliAdapter() }))
