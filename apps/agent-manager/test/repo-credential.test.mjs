...
// Windows CI (ticket e09fa003): 실제 git 으로 credential store 를 심고 POSIX 파일권한
// (mode & 0o077 === 0)·절대경로(startsWith '/')를 단언한다 — Windows 는 mode 비트·경로
// 규약이 달라 두 단언이 깨진다. Linux 개발 호스트에서 windows-latest 러너를 직접 관찰할
// 수 없어(검증 불가) win32 에서 명시적 skip. 같은 파일의 pure 테스트(maskCredential /
// authenticatedCloneUrl / 구조 가드)는 Windows 에서도 그대로 돈다. Windows 검증은 후속 티켓.
test('installRepoCredential: awb-credentials(owner 전용) + credential.helper=store 설치', {
  skip: process.platform === 'win32' && 'POSIX credential-file 권한/절대경로 단언 — windows-latest 미검증 (ticket e09fa003)',
}, async () => {
  ...
  try {
    ...
    assert.ok(credFile.startsWith('/'), `credential 파일이 절대경로가 아님: ${credFile}`);
    assert.equal(credFile, join(repo.root, '.git', 'awb-credentials'));
    // 내용에 인증 URL(토큰 포함), 파일권한은 owner 전용(그룹/other 비트 0).
