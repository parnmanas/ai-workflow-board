...
- **동시(overlapping) `dataSource.transaction()` 미지원 — sql.js**: sql.js 백엔드(`db.ts`의 `AppDataSource`)는 단일 WASM 인스턴스/단일 커넥션이라 진짜 커넥션 풀링이 없다. 두 호출이 같은 시점에 각각 `dataSource.transaction()`을 열면(예: 두 서비스 호출을 `Promise.all`로 동시 실행) 양쪽 다 "cannot start a transaction within a transaction" 또는 "Transaction is not started yet, start transaction before committing or rolling it back." 에러로 실패한다(outreach 인제스트 레이스 수정 중 발견, 티켓 2500fea3). Postgres는 풀에서 별도 커넥션을 받으므로 영향받지 않을 것으로 예상되나 별도 검증은 안 됨. sql.js 기준으로 새 동시성 테스트를 작성할 때는 하나의 트랜잭션 안에 여러 async 호출이 겹치는 설계를 피하거나, 트랜잭션 중첩 대신 claim-first + 실패 시 명시적 보상삭제(compensate) 패턴으로 우회하거나, Postgres 전용으로 스킵 처리할 것.
