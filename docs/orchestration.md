...
- **진행 중(in-flight) step 만** 방을 조회한다. 동시 실행은 `max_parallel_steps` 로 묶여
  ...
  행 역방향 탐색이다. 종료된 step 의 기록은 그 step 을 **선택했을 때만**
  `GET /api/orchestration/steps/:id/session` 으로 읽는다 — 카드 목록은 30초마다 다시
  그려지므로 여기에 전사를 실으면 그 비용이 step 수에 비례해 늘어난다.
...
### 미션 화면의 구조 (2단 + 선택이 내용을 가른다)

미션 화면은 한 페이지에 브리핑·계획·대화·타임라인을 전부 쌓아 두고 있었다. 그러면 스크롤
위치가 곧 맥락이라, 대화를 읽다가 step 상태를 보려면 화면을 잃어버린다. 지금은 2단이다:

- **왼쪽 레일** — "무엇을 볼지". 맨 위 한 줄이 미션 대화(orchestrator)이고 그 아래가
  단계(stage)로 묶인 step 목록이다. 진행 중인 행에만 활동 한 줄과 무신호 시계가 붙는다.
- **오른쪽 패널** — 고른 것의 내용. `Session` / `Plan graph` / `Brief` 세 탭이고, Session
  탭의 내용은 **선택 상태가 가른다**: step 을 고르면 그 step 의 작업 세션, 선택을 풀면
  (같은 행을 다시 누르거나 맨 위 Mission 행) 미션 대화.

그래서 탭이 네 개가 되지 않는다 — "미션 대화"와 "step 세션"은 같은 자리의 두 내용이고,
그 전환은 레일의 선택 하나로 표현된다. 위상 전체가 필요할 때만 Plan graph 탭이 넓은
그래프를 그리고(240px 열 여러 개는 레일 폭에 들어가지 않는다), 목표·완료 조건·계획 요약처럼
거의 변하지 않는 문서는 Brief 탭에 모았다.

**타임라인 섹션은 없앴다.** 실행 이벤트는 이미 미션 대화에 시간순으로 엮여 들어가고(사용자
지시 직후 무엇이 디스패치됐는지가 한 흐름에서 읽히는 것이 그 패널의 존재 이유다), step 을
선택하면 그 step 의 이벤트가 step 세션에 같은 방식으로 얹힌다. 같은 내용을 페이지 아래에 한
번 더 쌓아 두면 화면만 길어진다.

**step 세션은 읽기 전용이다.** step 방은 설계상 사람을 참여자로 넣지 않는다 — 입력창을
붙이면 존재하지 않는 능력을 약속하는 화면이 된다. 방향을 바꾸는 입구는 미션 대화 하나뿐이고,
그 이유를 패널 바닥에 한 줄로 적어 둔다. 읽기 경로는 `GET /api/orchestration/steps/:id/session`
...
화면도 구분해서 말한다: 바이트는 왔는데 브라우저가 디코드하지 못하면 썸네일·라이트박스가
"깨진 파일"로 표시한다 — 예전에는 영원히 "…" 자리표시자여서 "AWB 가 못 보여준다"로 읽혔다.
